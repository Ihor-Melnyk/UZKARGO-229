...
function onSearchBranch(searchRequest) {
  searchRequest.filterCollection.push({ attributeCode: "SubdivisionLevelDirect", value: "1" });
}
...
function onTaskExecuteEnterResults(routeStage) {
  if (routeStage.executionResult == "executed") {
    if (!EdocsApi.getAttributeValue("SpesificationResult").value) throw `Внесіть значення в поле "Результат розгляду ТУ засіданням"`;
    if (!EdocsApi.getAttributeValue("NumberProtocol").value) throw `Внесіть значення в поле "Номер протоколу"`;
    if (!EdocsApi.getAttributeValue("DateProtocol").value) throw `Внесіть значення в поле "Дата протоколу"`;
  }
}

function SignDocumentTask() {
  var stateTask = EdocsApi.getCaseTaskDataByCode("SignDocument")?.state;

  if (stateTask == "assigned" || stateTask == "inProgress" || stateTask == "delegated") {
    setPropertyHidden("AddDocument", false);
    setPropertyDisabled("AddDocument", false);
  } else if (stateTask == "completed") {
    setPropertyHidden("AddDocument", false);
    setPropertyDisabled("AddDocument");
  } else {
    setPropertyHidden("AddDocument");
    setPropertyDisabled("AddDocument", false);
  }
}

function onTaskExecuteSignDocument(routeStage) {
  if (routeStage.executionResult == "executed") {
    if (!EdocsApi.getAttributeValue("AddDocument").value) throw `Внесіть значення в поле "Додати виписку із держ. Реєстру"`;
  }
}
...
function onTaskExecuteRegisterSpesification(routeStage) {
  if (routeStage.executionResult == "executed") {
    if (!EdocsApi.getAttributeValue("RegDate").value) throw `Внесіть значення в поле "Реєстраційна дата"`;
    if (!EdocsApi.getAttributeValue("RegNumber").value) throw `Внесіть значення в поле "Реєстраційний номер"`;
    if (!EdocsApi.getAttributeValue("Registraion").value) throw `Внесіть значення в поле "Реєстрація"`;
  }
}

function onTaskExecutedRegisterSpesification(routeStage) {
  if (routeStage.executionResult == "executed") {
    SignDocumentTask();
  }
}
...
function onTaskExecuteRegisterAct(routeStage) {
  if (routeStage.executionResult == "executed") {
    if (!EdocsApi.getAttributeValue("PaymentOption").value) throw `Внесіть значення в поле "Вид оплати"`;
  }
}
...
function onTaskExecuteReceiptFunds(routeStage) {
  if (routeStage.executionResult == "executed") {
    if (!EdocsApi.getAttributeValue("StatusIn").value) throw `Внесіть значення в поле "Статус оплати Замовником"`;
  }
}
